...
export function Database(logger: Logger, options: { uri: string }) {
  ...
  async function getQueueIdByName(name: string) {
  ...
  async function getQueueByName(name: string) {
    const [rows] = await withConnection((connection) =>
  ...
  async function createQueue(params: DbCreateQueueParams) {
  ...
  async function updateQueue(params: DbUpdateQueueParams) {
    await withConnection((connection) => {
      return connection.query(
  ...
  async function addJobs(queueName: string, params: Omit<DbAddJobsParams, 'queueId'>[], connection?: Connection) {
  ...
  async function getPendingJobs(queueId: string, batchSize: number) {
    const [rows] = await withConnection((connection) =>
  ...
  async function markJobAsCompleted(connection: PoolConnection, jobId: string, currentAttempts: number) {
  ...
  async function incrementJobAttempts(
  ...
  async function markJobAsFailed(connection: PoolConnection, jobId: string, error: string, currentAttempts: number) {
    await connection.execute(
